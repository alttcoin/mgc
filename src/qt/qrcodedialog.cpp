...
{
    QString ret = QString("magneticoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non MGC unit input in lnReqAmount, we generate the URI with MGC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::MGC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
